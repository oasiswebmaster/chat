...
export function CreateItemModal({ type, onSubmit, onClose }: { type: 'folder' | 'document', onSubmit: (name: string) => void, onClose: () => void }) {
      ...
      >
        <div className="flex items-center justify-between px-6 py-4 border-b border-white/10 bg-white/5">
          <div className="flex items-center gap-3 text-gray-200">
            ...
            <h3 className="font-medium text-lg text-white">New {type === 'folder' ? 'Folder' : 'Document'}</h3>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-6">
          ...
          <div className="flex justify-end gap-2 text-gray-200">
            ...
            <button type="submit" disabled={!name.trim()} className="px-4 py-2 bg-white text-black font-medium text-sm rounded-lg hover:bg-gray-200 transition-colors shadow-sm disabled:opacity-50">Create</button>
          </div>
        </form>
      </motion.div>
    </div>
